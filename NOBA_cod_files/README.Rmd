...
# source(here::here("NOBA_cod_files", "Rscript", "comparison_figure.R"))
...
recnums <- YOY_ss %>%
  ...
  mutate(recnums = NCO.0/k_wetdry/truth$biolprm$redfieldcn) %>%
  filter(yr >0)
...
# R over SSB
plot(trueSSB_ss, recnums$recnums[which(as.integer(recnums$Time/73) %in% fit_timesteps)],
     xlab="True SSB", ylab="True R")
